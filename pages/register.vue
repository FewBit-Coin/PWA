...
useHeadSafe({
	title: `${t("title.register")} - ${t("appName")}`,
	meta: [{ name: "description", content: `${t("content.loginIntro")}` }],
});
...
<template>
	...
	>
		<div class="container px-10 mx-auto overflow-hidden">
			<div
				class="mx-5 pt-10 flex flex-col text-center md:text-left md:flex-row min-h-screen justify-center items-center"
			...
			>
				<div class="hidden lg:flex flex-col w-full">
					...
					<p class="w-8/12 mx-auto md:mx-0 mt-2">{{ $t('content.loginIntro') }}</p>
				</div>
				<div class="mx-auto flex justify-center items-center w-full">
					<div
						class="bg-light_blue-50/95 dark:bg-dark_blue-900/95 p-8 flex flex-col w-full shadow-xl rounded-xl"
					>
						<h2 class="text-2xl font-bold text-left mb-3">{{ $t('content.register') }}</h2>
							...
							<div class="grid grid-cols-1 md:grid-cols-2 gap-x-2">
								...
								<div class="flex flex-col w-full my-2">
									<InputOutline
										...
										type="password"
										:label="$t('content.passwordConfirmation')"
